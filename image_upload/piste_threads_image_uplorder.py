#!/usr/bin/env python3
"""
Piste_threads_image内の画像をGoogleドライブに移動するスクリプト
...
# 画像ディレクトリのパス
IMAGE_DIRECTORY = Path("/Users/ishikawasuguru/Threads_piste/Piste_threads_image")
...
SYSTEM_DIR = Path(__file__).parent.parent.parent / "90_System"
...
def find_credentials_file():
    ...
    # スクリプトのディレクトリ（Threads_piste）も検索対象に追加
    current_dir = Path(__file__).parent
    
    # 1. カレントディレクトリでcredentials.jsonを探す
    credentials_file = current_dir / "credentials.json"
    if credentials_file.exists():
        return credentials_file
    
    # 2. カレントディレクトリでclient_secret*.jsonを探す
    for file in current_dir.glob("client_secret*.json"):
        return file

    # 3. Systemディレクトリでcredentials.jsonを探す
    credentials_file = SYSTEM_DIR / "credentials.json"
    if credentials_file.exists():
        return credentials_file
    
    # 4. Systemディレクトリでclient_secret*.jsonを探す
