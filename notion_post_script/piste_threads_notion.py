#!/usr/bin/env python3
"""
piste_threads_post.mdの内容をNotionデータベースに転記するスクリプト
...
def parse_markdown_posts(markdown_content):
    """Markdownファイルから投稿案を抽出"""
    posts = []
    
    # 投稿案を分割（## で始まるセクション）
    sections = re.split(r'^## ', markdown_content, flags=re.MULTILINE)
    
    for section in sections[1:]:  # 最初のセクション（# Threads投稿案）をスキップ
        if not section.strip():
            continue
        
        lines = section.split('\n')
        # 投稿案のタイトルを抽出（例: "投稿案1: コンビニランチ編"）
        title_line = lines[0].strip()
        title_match = re.match(r'投稿案\d+:\s*(.+)', title_line)
        if title_match:
...
def main():
    ...
    try:
        ...
        # フィルタリング: 2026/02/23 〜 3/01
        target_start = datetime(2026, 2, 23).date()
        target_end = datetime(2026, 3, 1).date()
        ...
        print(f"✓ フィルタリング適用 (2/23 - 3/1): {len(posts)} 件の対象投稿")
